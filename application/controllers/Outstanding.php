...
class Outstanding extends CI_Controller {
    ...
    public function admin_outstanding(){
      
		if ($this->accesscontrol->checkAuth()['correct']) {

			$this->load->view('shared/headerSuperAdmin');
		    $this->load->view('admin/admin_outstanding');
  ...
  public function update_not_image()
  
    {
      if ($this->accesscontrol->checkAuth()['correct']) {
        ...
        if (!$valid) {
          ...
          if ($res = $this->OutstandingModel->update_not_image($data)) {
  ...
  
    {
      if ($this->accesscontrol->checkAuth()['correct']) {
        ...
        if (!$valid) {
          ...
          if ($res = $this->OutstandingModel->update_outstanding($data)) {
    ...
    public function upImage($id_image){
        if ($this->accesscontrol->checkAuth()['correct']) {
              ...
              $config['upload_path'] = "./assets/images/outstanding";
              ...
              $this->upload->initialize($config);
                  ...
                  if ($this->upload->do_upload("file-section")) {
                    ...
                    $this->load->model('OutstandingModel');
                    $s = $this->OutstandingModel->up_image($id_image, $url);
